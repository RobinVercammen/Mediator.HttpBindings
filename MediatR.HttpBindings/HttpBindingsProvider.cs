...
namespace MediatR.HttpBindings
{
    public class HttpBindingsProvider : IApplicationFeatureProvider<ControllerFeature>
    {
        ...
        public void PopulateFeature(IEnumerable<ApplicationPart> parts, ControllerFeature feature)
        {
            ...
            foreach (var requestType in requests)
            {
                ...
                if (!feature.Controllers.Any(t => t.Name == controllerName))
                {
                    ...
                    var controllerType = typeof(RequestResponseController<,>).MakeGenericType(requestType, responseType);
                    ...
                    methodBuilder.DefineParameter(1, parameter.Attributes, parameter.Name);
        ...
        {
            return t.GetInterfaces().Single(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IRequest<>)).GetGenericArguments()[0];
        }
        ...
        private static TypeBuilder CreateControllerTypeBuilder(string controllerName, Type controllerType)
        {
            ...
            var typeBuilder = moduleBuilder.DefineType(controllerName, System.Reflection.TypeAttributes.Public, controllerType);
            return typeBuilder;
        }
        ...
        {
            return controllerType.GetMethod("ExecuteAsync", BindingFlags.Instance | BindingFlags.NonPublic, Type.DefaultBinder, new[] { requestType }, null);
        }

        private static MethodBuilder CreateHandleMethod(TypeBuilder typeBuilder, MethodInfo method)
        {
            var parameters = method.GetParameters().Select(p => p.ParameterType).ToArray();
            return typeBuilder.DefineMethod("Handle", MethodAttributes.Public, method.CallingConvention, method.ReturnType, parameters);
        }

        private static void CallBaseMethod(MethodBuilder methodBuilder, ParameterInfo[] parameters, MethodInfo baseMethod)
        {
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                ilCode.Emit(OpCodes.Ldarg_S, i + 1); // specify index
            }
